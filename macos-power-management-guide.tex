...
\large MacBook Pro 16,2 (2020) Troubleshooting Reference}
\author{System Configuration Documentation}
...
\midrule
Touch ID not working & \texttt{sudo killall biometrickitd BiomeAgent biomed TouchBarServer} \\
\addlinespace
Check sleep blockers & \texttt{pmset -g assertions} \\
\addlinespace
Check thermal status & \texttt{pmset -g therm} \\
\addlinespace
Monitor throttler & \texttt{tail -f /tmp/icloud-throttle.out} \\
\addlinespace
Kill runaway process & \texttt{kill -9 <PID>} \\
\addlinespace
Reset iCloud sync & \texttt{killall -9 fileproviderd cloudd bird} \\
\bottomrule
\end{tabular}
